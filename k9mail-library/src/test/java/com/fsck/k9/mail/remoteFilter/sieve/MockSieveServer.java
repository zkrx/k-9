...
public class MockSieveServer {
    ...
    private final Deque<SieveInteraction> interactions = new ConcurrentLinkedDeque<>();
    ...
    MockSieveServer(KeyStoreProvider keyStoreProvider, Logger logger) {
    ...
    void expect(String command) {
        checkServerNotRunning();
        interactions.add(new ExpectedCommand(command));
    }

    void expectAnyString() {
        checkServerNotRunning();
        interactions.add(new ExpectAnyCommand());
    }
    ...
    void waitForInteractionToComplete() {
        checkServerRunning();
    ...
    void verifyInteractionCompleted() {
        shutdown();
    ...
    void verifyConnectionStillOpen() {
        checkServerRunning();
    ...
    private interface SieveInteraction {}

    private static class ExpectedCommand implements SieveInteraction {
        private final String command;


        ExpectedCommand(String command) {
    ...
    private static class ExpectAnyCommand implements SieveInteraction {
        ExpectAnyCommand() {
        }
    }

    private static class CannedResponse implements SieveInteraction {
        private final String response;


        CannedResponse(String response) {
    ...
    private static class CloseConnection implements SieveInteraction {
    }

    private static class EnableCompression implements SieveInteraction {
    }

    private static class UpgradeToTls implements SieveInteraction {
    }

    private static class UnexpectedCommandException extends Exception {
        UnexpectedCommandException(String expectedCommand, String receivedCommand) {
    ...
    private static class MockServerThread extends Thread {
        ...
        private final Deque<SieveInteraction> interactions;
        ...
        private void handleInteractions(Socket socket) throws IOException, KeyStoreException,
            ...
            SieveInteraction interaction = interactions.pop();
            if (interaction instanceof ExpectedCommand) {
                readExpectedCommand((ExpectedCommand) interaction);
            } else if (interaction instanceof ExpectAnyCommand) {
                readExpectAnyCommand();
        ...
        private void readExpectAnyCommand() throws IOException,
                UnexpectedCommandException {
        ...
        boolean isClientConnectionClosed() {
            return clientSocket.isClosed();
        }

        UnexpectedCommandException getUnexpectedCommandException() {
