...
class MessageListExtractor(
    ...
    private fun extractMessageListItem(cursor: Cursor, uniqueIdColumn: Int): MessageListItem {
        ...
        val threadCount = cursor.getInt(MLFProjectionInfo.THREAD_COUNT_COLUMN)
        val subject = cursor.getString(MLFProjectionInfo.SUBJECT_COLUMN)
        val isRead = cursor.getInt(MLFProjectionInfo.READ_COLUMN) == 1
        val isStarred = cursor.getInt(MLFProjectionInfo.FLAGGED_COLUMN) == 1
        val isAnswered = cursor.getInt(MLFProjectionInfo.ANSWERED_COLUMN) == 1
        val isForwarded = cursor.getInt(MLFProjectionInfo.FORWARDED_COLUMN) == 1
