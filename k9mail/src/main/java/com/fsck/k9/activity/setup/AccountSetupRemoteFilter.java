...
public class AccountSetupRemoteFilter extends K9Activity implements OnClickListener {
    private static final String EXTRA_ACCOUNT = "account";
    private static final String EXTRA_REMOTE_FILTER_TYPE = "remoteFilterType";
    ...
    public static void actionEditRemoteFilterSettings(Activity context, Account account, ServerSettings.Type type) {
        context.startActivity(intentActionRemoteFilterSettings(context, account, type));
    }

    public static Intent intentActionRemoteFilterSettings(Context context, Account account, ServerSettings.Type type) {
        Intent i = new Intent(context, AccountSetupRemoteFilter.class);
        i.setAction(Intent.ACTION_EDIT);
        i.putExtra(EXTRA_ACCOUNT, account.getUuid());
        i.putExtra(EXTRA_REMOTE_FILTER_TYPE, type.name());
        return i;
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.account_setup_remote_filter);
        ...
        try {
            ...
            if (mAccount.getRemoteFilterStoreUri() != null) {
                settings = RemoteFilterStore.decodeStoreUri(mAccount.getRemoteFilterStoreUri());
            } else {
                ServerSettings incomingServer = RemoteMailStore.decodeStoreUri(mAccount.getStoreUri());
                settings = new ServerSettings(ServerSettings.Type.valueOf(getIntent().getStringExtra(EXTRA_REMOTE_FILTER_TYPE)),
                        incomingServer.host, -1, incomingServer.connectionSecurity, incomingServer.authenticationType,
                        incomingServer.username, incomingServer.password, incomingServer.clientCertificateAlias);
            }
            ...
            if (Type.SIEVE == settings.type) {
                serverLabelView.setText(R.string.account_setup_remote_filter_sieve_server_label);
            } else {
    ...
    private void validateFields() {
        ...
        if (isAuthTypeExternal && !hasConnectionSecurity) {
            ...
            String toastText = getString(R.string.account_setup_remote_filter_invalid_setting_combo_notice,
    ...
    protected void onNext() {
        try {
            ...
            mAccount.deleteCertificate(host, port, CheckDirection.REMOTE_FILTER);
            ServerSettings settings = new ServerSettings(mStoreType, host, port,
                    connectionSecurity, authType, username, password, clientCertificateAlias, null);

            mAccount.setRemoteFilterStoreUri(RemoteMailStore.createStoreUri(settings));

            AccountSetupCheckSettings.actionCheckSettings(this, mAccount, CheckDirection.REMOTE_FILTER);
        } catch (Exception e) {
            failure(e);
        }
    ...
    private ConnectionSecurity getSelectedSecurity() {
        ...
        return holder != null ? holder.connectionSecurity : null;
    }
}
