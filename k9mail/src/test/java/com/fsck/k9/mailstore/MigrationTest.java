...
public class MigrationTest {
    ...
    public void migrateTextPlain() throws Exception {
        ...
        Assert.assertEquals(1, msg.getUnparsedHeader("User-Agent").length);
        Assert.assertEquals("Mutt/1.5.24 (2015-08-30)", msg.getUnparsedFirstHeader("User-Agent"));
        Assert.assertEquals(1, msg.getUnparsedHeader(MimeHeader.HEADER_CONTENT_TYPE).length);
        Assert.assertEquals("text/plain",
                MimeUtility.getHeaderParameter(msg.getUnparsedFirstHeader(MimeHeader.HEADER_CONTENT_TYPE), null));
        Assert.assertEquals("utf-8",
                MimeUtility.getHeaderParameter(msg.getUnparsedFirstHeader(MimeHeader.HEADER_CONTENT_TYPE), "charset"));
    ...
    public void migrateMixedWithAttachments() throws Exception {
        ...
        Assert.assertEquals("----5D6OUTIYLNN2X63O0R2M0V53TOUAQP",
                MimeUtility.getHeaderParameter(msg.getUnparsedFirstHeader(MimeHeader.HEADER_CONTENT_TYPE), "boundary"));
    ...
    public void migratePgpMimeEncryptedMessage() throws Exception {
        ...
        Assert.assertEquals("application/pgp-encrypted",
                MimeUtility.getHeaderParameter(msg.getUnparsedFirstHeader(MimeHeader.HEADER_CONTENT_TYPE), "protocol"));
